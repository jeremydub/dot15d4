...
/// Guard time used by the driver to go from any radio state to TX ready.
const TX_GUARD_TIME_US: usize =
    (OFFSET_DIS_TO_TX_W_CCA.checked_add(T_RXDIS).unwrap()).to_micros() as usize;
...
impl DriverConfig for NrfRadioDriver {
    ...
    type TxGuardTime = U<TX_GUARD_TIME_US>;
